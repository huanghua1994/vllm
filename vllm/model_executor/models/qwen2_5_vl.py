...
class Qwen2_5_VisionBlocksRunner(nn.Module):
    def __init__(
        self,
        ...
        super().__init__()
        # Keep the canonical parameter names under Qwen2_5_VisionTransformer.blocks.
    ...
    def forward(
        ...
        blocks = cast(nn.ModuleList, self.blocks)
...
class Qwen2_5_VisionTransformer(nn.Module):
    def __init__(
        ...
        self.use_blocks_loop_compiler_runner = os.getenv(
            "VLLM_QWEN2_5_VL_COMPILE_BLOCKS_LOOP", "1"
        ).lower() in ("1", "true", "yes")
        if self.use_blocks_loop_compiler_runner:
            logger.warning_once(
                "Using experimental Qwen2.5-VL vision blocks loop compiler "
                "runner. Disable with VLLM_QWEN2_5_VL_COMPILE_BLOCKS_LOOP=0."
            )
            with set_model_tag("Qwen2_5_VisionBlocksRunner", is_encoder=True):
                self.blocks_runner = Qwen2_5_VisionBlocksRunner(
                    blocks=self.blocks,
    ...
    def forward(
        ...
        if self.use_blocks_loop_compiler_runner:
            hidden_states = self.blocks_runner(
                hidden_states,
                cu_seqlens=cu_seqlens,
